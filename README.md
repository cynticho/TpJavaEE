...
in this project, we build a photographer's management system in which, administrator can manage photographer namelly make CRUD(Create, Read, Update, Delete).
In the same way, after added photographer, they can be conneted to their account and manage their image such as CRUD.
To launch this app, navigate to the target folder and run the comman java -jar ./javaproject-1.0-SNAPSHOT
After this, the app will run on port 8081, we just need to enter localhost:8081 in the browser to access them
=======
# tpJavaEE
...
docker build . -t javaproject:latest
```

Once the Docker image is correctly built, you can test it locally using

```
docker run -p 8080:8080 javaproject:latest
```
...
If the pods say `Container image "javaproject:latest" is not present with pull policy of Never` then you have not built your application using Docker or there is a mismatch in the name. Use `docker images ls` to see which images are available.
